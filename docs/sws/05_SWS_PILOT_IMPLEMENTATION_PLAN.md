...
| Version / status | v0.2.1 — Focused minor revision; ready for final verification; not frozen |
...
- `execution_owner` must not be the only `professional_reviewer` for the same Package.
- `execution_owner` must not be the only `report_back_approver` for the same Package.
- Every Package must retain at least one independent reviewer who did not perform the Package's primary execution work.
- Any approved role-overlap exception must record the reason, approver, applicable Scope, expiry or review trigger, and the remaining independent reviewer.
- A role-overlap exception must not remove all independent review.
- If the minimum independent-review requirement cannot be satisfied, the Package remains Blocked and must not enter Passed.
- These rules do not assign or infer any currently Missing person.
